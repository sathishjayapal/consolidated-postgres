...
project's local password is rotated (e.g. `jubilant-memory/config/.env`
changes) and you redeploy, Postgres will happily keep running under its *old*
role — `POSTGRES_USER`/`PASSWORD` only apply the first time a volume
initializes. Run `scripts/vm/check-stack-consistency.sh --roles` after any
credential rotation to catch this before it blocks you.
