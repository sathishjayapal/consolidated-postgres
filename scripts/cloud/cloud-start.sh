...
#
# Usage: ./scripts/cloud/cloud-start.sh
#
# What it does:
...
#   4. Generates cloud connection details to env/.env.cloud
...
#
# Next steps:
#   1. Update application.yml in each project with env/.env.cloud details
#   2. Deploy your applications (or run locally pointing to cloud DB)
#   3. When done: ./scripts/cloud/cloud-stop.sh (exports data and destroys infrastructure)
#################################################################
...
echo ""
print_info "Remember: Run ./scripts/cloud/cloud-stop.sh when done coding to stop charges!"
echo ""
...
# Create env/.env.cloud file
print_section "Generating env/.env.cloud Configuration"

cat > "$ENV_CLOUD" << 'EOF'
# Cloud Database Configuration
# Generated by scripts/cloud/cloud-start.sh
...
# Append configuration values safely (using printf to handle special characters)
{
  ...
  printf "# Action: Run ./scripts/cloud/cloud-stop.sh to stop charges and export data\n"
} >> "$ENV_CLOUD"

print_status "Created $ENV_CLOUD"
print_info "Copy connection details from above to your applications"

# Create reminder file
cat > "$CLOUD_STATUS_FILE" << EOF
...
⏰ Don't forget to run ./scripts/cloud/cloud-stop.sh when done coding!
EOF

print_status "Status file: $CLOUD_STATUS_FILE"
...
echo ""
echo "Next Steps:"
echo "  1. Copy connection details from env/.env.cloud to each application"
...
echo "   ./scripts/cloud/cloud-stop.sh"
echo ""
echo "This will:"
