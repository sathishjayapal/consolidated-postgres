...
#
# Usage: ./scripts/acg/acg-start.sh
#
# What it does:
#   1. Reads ACG sandbox details (subscription_id, rg_name)
#      from acg.tfvars (create from env/acg.tfvars.example) or prompts
...
#   4. Writes env/.env.acg in this directory
#   5. Updates each project's .env file with the new acg DB settings
#
# Prerequisites:
#   - Terraform installed
#   - Azure CLI logged in (az login) OR ARM_* env vars set
#   - acg.tfvars exists with your sandbox credentials (see env/acg.tfvars.example)
...
#   ./scripts/local/multi-dev-up.sh --acg
#   (starts apps pointing at the ACG DB, skips local containers)
#################################################################
...
# per-project "domain A" (project .env / app-runtime) keys used by local dev
# and by each project's dev-up.sh --acg, so this file and dev-up.sh never
# disagree on what a key is called.
print_section "Writing env/.env.acg"

{
  printf "# ACG Azure PostgreSQL — generated by scripts/acg/acg-start.sh\n"
  ...
  for entry in "eventstracker:$JDBC_ET" "runs-app:$JDBC_RA" "runs-ai-analyzer:$JDBC_AI" \
    ...
    printf "%s=%s\n" "$(get_project_db_user_env_key "$project")" "$PG_USER"
    printf "%s=%s\n\n" "$(get_project_env_password_key "$project")" "$PG_PASS"
  done
} > "$ENV_ACG"
